...
def start_callback(update: Update, context: CallbackContext):
    """Послать сообщение, когда была послана команда ``/start``."""
    message = update.message
    message.reply_markdown(REPLIES.start)


def help_callback(update: Update, context: CallbackContext):
    """Послать сообщение, когда была послана команда ``/help``."""
    message = update.message
...
def reply_callback(update: Update, context: CallbackContext):
    """
    Данная ручка предназначена для посылки ответов.
    Можно из чатика собирателей обратной связи ответить на любое пересланное сообщение и ответ будет послан
...
def forward_callback(update: Update, context: CallbackContext):
    """Переслать сообщение по адресу ``CHAT_ID``. Там должны находиться собиратели информации."""
